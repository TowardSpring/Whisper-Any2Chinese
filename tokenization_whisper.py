...
# Changed the part of following code by Towardspring due to adapt the new function of Whisper model
...
from config.whisper_config import MODEL_CONFIG_FILE_ROOT_PATH, TRANSLATED_CHINESE_TOKEN, LANGUAGE_TOKEN
...
TASK_IDS = ["translate", "transcribe", "translate-chinese"]
ADDED_TOKENS = os.path.join(MODEL_CONFIG_FILE_ROOT_PATH, "added_tokens.json")
...
class WhisperTokenizer(PreTrainedTokenizer):
    ...
    def get_added_tokens(self):
        # 获取特殊符号的对应id号码
        with open(ADDED_TOKENS,'r', encoding='utf-8') as obj:
            lines = json.load(obj)
        return lines
    ...
    def prefix_tokens(self) -> List[int]:
        added_tokens_ids = self.get_added_tokens()
        ...
        translate_chinese_token = '<|translate-chinese|>'

        bos_token_id = added_tokens_ids[bos_token]
        translate_chinese_id = added_tokens_ids[translate_chinese_token]
        translate_token_id = added_tokens_ids[translate_token]
        transcribe_token_id = added_tokens_ids[transcribe_token]
        notimestamps_token_id = added_tokens_ids[notimestamps_token]
        ...
        if self.language is not None:
            ...
            language_index = bos_token_id + 1 + langs.index(language_id)
            bos_sequence.append(added_tokens_ids[f'<|{language_id}|>'])
        if self.task is not None:
            # bos_sequence.append(transcribe_token_id if self.task == "transcribe" else translate_token_id)
            if self.task == "translate":
                bos_sequence.append(transcribe_token_id)
            elif self.task == "translate-chinese":
                bos_sequence.append(translate_chinese_id)
            else:
